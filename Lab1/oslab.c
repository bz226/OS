...
// TODO: You need to implement th main loop below:
time = 0;
while(num_completed_processes < num_processes){
	...
	/* If there is a process, (ie, scheduler does not return -1):  
	  - Increment cpu_active
	  - Update the state of the chosen process from ready to running.
	  - Print information in the output file as indicated in the lab description. 
	  - If needed, update num_completed_processes
	  */
	...
	// Print in the output file the information about non-running processes that exist in the system.
	print_processes(output, time, running_process_index);
    // Update the status of blocked processes if needed. 
